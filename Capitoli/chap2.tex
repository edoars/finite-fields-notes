...
\begin{proof}
	\graffito{\("\Rightarrow"\)}If \(f\) is primitive then it is irreducible over \(F_q\) and, by \autoref{3.3}, its order is the order of one of its roots \(\a\) over \(\F_{q^m}\), which is \(q^m-1\) as \(\a\) is a primitive element of \(\F_{q^m}\) over \(\F_q\).
